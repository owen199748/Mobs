...
public class WorldSpawn extends Spawn{
	...
	public void setPlayerNearby(int playerNearby) {
		...
		public WorldSpawn(String sName,ConfigurationSection cfg,ArrayList<World> world) {
			super(sName,cfg);
this.world=world;
chance=5;
playerNearby=16;
Wmobcreates.add(this);
	...
	ArrayList<Location> loc = new ArrayList<Location>();
			for(int s=0;s<getWorld().size();s++){
				...
				if(is()){
					int x = p.getLocation().getBlockX()+( ((int)(Math.random()*(getPlayerNearby()*2)))-getPlayerNearby() );
					int z = p.getLocation().getBlockZ()+( ((int)(Math.random()*(getPlayerNearby()*2)))-getPlayerNearby() );
					int y1=-1;
					int y2=-1;
					int y=-1;
					for(int i=p.getLocation().getBlockY();i<=w.getMaxHeight();i++){
						if (w.getBlockAt(x, i, z).getType().name().equalsIgnoreCase(Material.AIR.name()));
						{if(!w.getBlockAt(x, i-1, z).getType().name().equalsIgnoreCase(Material.AIR.name()))
							{y1=i;
							break;
							}
							}
					...
					if(Math.abs(y1-p.getLocation().getBlockY())>Math.abs(y2-p.getLocation().getBlockY())){
						y=y2;
					}else {
						y=y1;
					}
